...
def detect_objects(model, frame, class_names, display_width):
    """Dectect segmentation masks, draw bounding boxes, and overlay labels."""
    ...
    for r in results:
        if r.masks: # If segmentation masks are detected
            ...
            for i, mask in enumerate(masks):
                # Resize the mask to match the resized frame dimensions
        ...
        if r.boxes: # If bounding boxes and detected
            for box in r.boxes:
                x1, y1, x2, y2 = map(int, box.xyxy[0]) # Bounding box coordinates
                conf = round(box.conf[0].item(),2) # Confidence score
                cls = int(box.cls[0]) # Class ID
                label = f"{class_names[cls]} {conf}" # Label with class name and confidence
...
def process_video_frame(model, frame, class_name, display_width):
    """Process a single video frame for segmentation"""
    frame_with_objects = detect_objects(model, frame, class_name, display_width)
